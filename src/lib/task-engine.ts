...
function missingTask({
  ...
  return makeTask({
    ...
    nextAction,
    whatThisMeans: "Business Next cannot calculate this deadline until the missing business detail is saved.",
...
export function buildDeadlineTasks(profile: BusinessProfile, today = new Date()): GeneratedTask[] {
  ...
  if (profile.businessType === "LIMITED_COMPANY") {
    ...
    if (!corporationTaxStart || !accountsEnd) {
      ...
      tasks.push(makeTask({
        ...
        whatThisMeans: "Corporation Tax is tax on company profits. Business Next only tracks the deadline; it does not calculate or submit the payment.",
  ...
  if (profile.businessType === "SOLE_TRADER") {
    if (!startedTradingOn) {
      ...
      tasks.push(makeTask({
        ...
        calculationInput: { taxYearEnd: format(deadlines.taxYearEnd, "yyyy-MM-dd") },
        calculationExplanation: `GOV.UK says the second payment deadline is 31 July where payments on account apply. Business Next cannot know from your profile whether HMRC will require them, so this is a check task.`,
        missingInformation: [],
  ...
  if (profile.employsPeople === "YES" || profile.worksAlone === "NO") {
    ...
    tasks.push(makeTask({
      ...
      nextAction: dueDate ? "Register before the first payday." : "Add the first payday so Business Next can show the deadline.",
      ...
      calculationExplanation: firstPayday
        ? `GOV.UK says to register before the first payday. Business Next shows ${format(firstPayday, "d MMMM yyyy")} as the latest date to handle this.`
